...
void dspic33epXXXmu::send_cmd(uint32_t cmd)
{
	uint8_t i;
...
inline void dspic33epXXXmu::send_prog_nop(void)
{
	uint8_t i;
...
uint16_t dspic33epXXXmu::read_data(void)
{
	uint8_t i;
	uint16_t data = 0;
...
/* enter program mode */
void dspic33epXXXmu::enter_program_mode(void)
{
	int i;
...
/* exit program mode */
void dspic33epXXXmu::exit_program_mode(void)
{
	GPIO_CLR(pic_clk);
	GPIO_CLR(pic_data);
	delay_us(DELAY_P16);
...
bool dspic33epXXXmu::read_device_id(void)
{
	bool found = 0;
...
uint8_t dspic33epXXXmu::blank_check(void)
{
	uint32_t addr;
	unsigned short i;
...
/* Bulk erase the chip */
void dspic33epXXXmu::bulk_erase(void)
{
...
void dspic33epXXXmu::read(char *outfile, uint32_t start, uint32_t count)
{
...
void dspic33epXXXmu::write(char *infile)
{
	uint16_t i,j,p;
	uint16_t k;
	bool skip;
...
void dspic33epXXXmu::dump_configuration_registers(void)
{
	const char *regname[] = {"FGS","FOSCSEL","FOSC","FWDT","FPOR",
							"FICD","FAS","FUID0"};
